  ...
  return (
    <div id="chapitres" className="w-full py-16 px-6 sm:px-10 lg:px-12 bg-[#fdfbf7] relative overflow-hidden h-full">
      ...
      <div className="relative z-10 max-w-2xl mx-auto">
        ...
        <div className="relative border-l-2 border-medium-dark-green/10 ml-4 sm:ml-6 space-y-8 sm:space-y-12 pb-12">
          {CHAPTERS.map((chapter, index) => (
            <div key={index} className="relative pl-8 sm:pl-12 group">
              {/* Timeline dot */}
              <div className="absolute -left-[9px] top-6 w-4 h-4 rounded-full border-2 border-white bg-medium-dark-green/30 group-hover:bg-heading-accent group-hover:scale-125 transition-all duration-300 z-10" />
              ...
              >
                ...
                <div className="absolute top-1/2 right-6 -translate-y-1/2 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 translate-x-4 transition-all duration-500 text-heading-accent pointer-events-none">
                  <Play className="w-10 h-10 fill-current opacity-10" />
                </div>
                ...
                <h3 className="text-xl sm:text-2xl font-normal mb-2 text-dark-green group-hover:text-heading-accent transition-colors duration-300 pr-8" style={{ fontFamily: '"Neue Haas Grotesk Display Pro 55 Roman", sans-serif' }}>
                  {chapter.title}
                </h3>
                
                <p className="text-sm sm:text-base text-body-green/80 font-inter leading-relaxed max-w-[90%]">
                  {chapter.text}
                </p>
              </button>
            </div>
